...
const FilterSidebar = () => {
    ...
    const materials = [
        "Cotton",
        "Wool", "Polyester", "Denim", "Silk", "Linen",
        "Viscose", "Fleece"
    ]

    const brands = ["Urban Threads",
        "Modern Fit", "Street Style", "Beach Breeze", "ChicStyle", "Fashionista"
    ]
    ...
    const updateURLParams = (newFilters) => {
        ...
        navigate(`?${params.toString()}`) //?category=Bottom+Wear&size=XS%
    }
        ...
        <div className='p-4'>
            ...
            <div className='mb-6'>
                ...
                <div className='flex flex-wrap gap-2 w-full' >
                    {
                        colors.map((color) => (
                            <button key={color} name='color' value={color} onClick={handleFilterChange} className={`block w-8 h-8 rounded-full border border-gray-300 cursor-pointer transition
                            hover:scale-105 ${filters.color === color ? 'ring-2 ring-blue-500' : ""}`} style={{ backgroundColor: color.toLowerCase() }}>
                    ...
                    materials.map((material) => (
                        < div key={material} className='flex items-center mb-1' >
                            <input type="checkbox" name='material' value={material}
                                checked={filters.material.includes(material)} onChange={handleFilterChange} className='mr-2 h-4 w-4 text-blue-500 focus:ring-blue-400 border-gray-300' />
                    ...
                    brands.map((brand) => (
                        < div key={brand} className='flex items-center mb-1' >
                            <input type="checkbox" name='brand' value={brand}
                                checked={filters.brand.includes(brand)} onChange={handleFilterChange} className='mr-2 h-4 w-4 text-blue-500 focus:ring-blue-400 border-gray-300' />
            ...
            <div className='mb-8'>
                ...
                <input type="range" name='priceRange'
                    ...
                    min={0} max={200} className='w-full h-2 bg-gray-300 appearance-none cursor-pointer rounded-lg' />
